...
# =====================================================
# 5. 🌐 HOME CREDIT 3D DATA GLOBE
# =====================================================
st.subheader("🌐 Home Credit 3D Data Globe")

st.markdown("""
**Interactive financial data globe:**  
Drag to rotate • Scroll to zoom • Hover over points to inspect applications
""")

# -----------------------------------------------------
# Select columns required for the globe
# -----------------------------------------------------
...
globe_df = globe_df[
    (globe_df["AMT_CREDIT"] >= 0)
    &
    (globe_df["AMT_INCOME_TOTAL"] >= 0)
    &
    (globe_df["AMT_ANNUITY"] >= 0)
]

# Limit points for better performance
globe_df = globe_df.sample(
    n=min(3000, len(globe_df)),
    random_state=42
...
if not globe_df.empty:
    ...
    credit_norm = safe_normalize(
        globe_df["AMT_CREDIT"]
    )

    income_norm = safe_normalize(
        globe_df["AMT_INCOME_TOTAL"]
    )

    annuity_norm = safe_normalize(
        globe_df["AMT_ANNUITY"]
    )


    # -------------------------------------------------
    # Convert values into spherical coordinates
    # -------------------------------------------------
    ...
    fig_globe.add_trace(
        go.Scatter3d(
            ...
            hovertemplate=(
                "<b>💳 Home Credit Application</b><br><br>"
                "Customer ID: %{customdata[0]}<br>"
                "Credit Amount: %{customdata[1]:,.0f}<br>"
                "Income: %{customdata[2]:,.0f}<br>"
                "Annuity: %{customdata[3]:,.0f}<br>"
                "TARGET: %{customdata[4]}"
                "<extra></extra>"
            )
        )
    ...
    fig_globe.update_layout(
        ...
        title=dict(
            text="🌐 Home Credit 3D Data Globe",
            x=0.5,
    ...
    st.warning(
        "Not enough valid data available to create the 3D globe."
    )
