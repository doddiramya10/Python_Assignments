...
def bar_chart(df:pd.DataFrame,group_col:str,value_col:str|None,title:str,top_n:int=None,aggfunc:str="sum"):
   ...
   fig = px.bar(data,x=group_col, y=sort_col,title=title,text=sort_col,color=sort_col,color_continuous_scale=["#DCEEFF","#8EC5FF","#4A90E2", "#1F5FAF"])
   fig.update_traces( texttemplate="%{text:,0f}",
        textposition="outside",
        marker_line_width=1,
        marker_line_color="#1F5FAF"
    )
   fig.update_layout(
        title_x=0.5,
...
def horizontal_bar_chart(
    ...
    fig.update_layout(
        title_x=0.5,
        xaxis_title="Applications" if value_col is None else value_col,
